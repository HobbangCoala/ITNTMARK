    ...
    public class VisionServer// : ITNTTCPServer
    {
        ...
        private async void AcceptCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                ITNTTraceLog.Instance.Trace(0, "{0}::{1}()  {2}", className, funcName, "listener Remote = " + listener.Server.RemoteEndPoint.ToString());
                ITNTTraceLog.Instance.Trace(0, "{0}::{1}()  {2}", className, funcName, "listener Local = " + listener.Server.LocalEndPoint.ToString());

                ITNTTraceLog.Instance.Trace(0, "{0}::{1}()  {2}", className, funcName, "RemoteEndPoint = " + connClients.socket.Client.RemoteEndPoint.ToString());
                ITNTTraceLog.Instance.Trace(0, "{0}::{1}()  {2}", className, funcName, "LocalEndPoint = " + connClients.socket.Client.LocalEndPoint.ToString());
            ...
            try
            {
                ...
                if (readCount > 0)
                {
                    ...
                    if (retval > 0)
                    {
                        ...
                        ITNTTraceLog.Instance.Trace(0, "{0}::{1}()  {2}", className, funcName, "RECV END");
                    }
                    else
