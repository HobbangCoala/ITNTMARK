...
namespace ITNTMARK
{
    ...
    public class PLCControlManager
    {
        ...
        {
            string value = "";
            Util.GetPrivateProfileValue("PLCCOMM", "PLCTYPE", "1", ref value, ITNTCOMMON.Constants.PARAMS_INI_FILE);
            int type = 0;
            Int32.TryParse(value, out type);
            plcType = (PLC_DEVICE_TYPE)type;

            Util.GetPrivateProfileValue("PLCCOMM", "COMMTYPE", "1", ref value, ITNTCOMMON.Constants.PARAMS_INI_FILE);
            Int32.TryParse(value, out type);
            plcCommType = (PLC_COMM_TYPE)type;

            melseqPLCComm = new PLCCommMELSEQ(callback);
            abPLCComm = new PLCCommABPLC(callback);
            siemensComm = new PLCCommSIEMENS(callback);
            lsPLCComm = new PLCCommLSPLC(callback);
            callbackHndler = callback;
        }

        public async Task<int> OpenPLCAsync()
        {
            int retval = 0;
            if (bPLCOpened)
                return 0;
...
//                retval = melseqPLCComm.ClosePLC(plcCommType);
        ...
        public async Task<ITNTResponseArgs> ReadPLCAsync(ITNTSendArgs sendArg)
        {
            ...
            if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_AB)
                ...
                retval = await siemensComm.ReadPLCAsync(sendArg);
            else if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_LS)
        ...
        public async Task<ITNTResponseArgs> WritePLCAsync(ITNTSendArgs sendArg)
        {
            ...
            if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_AB)
                ...
                retval = await siemensComm.WritePLCAsync(sendArg);
            else if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_LS)
        ...
        public async Task<ITNTResponseArgs> SendPLCValue2PLC(string value)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
            if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_AB)
                retval = await abPLCComm.SendPLCValue2PLC(value);
            else if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_MELSEC)
                retval = await melseqPLCComm.SendPLCValue2PLC(value);
            else if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_SIMENS)
                ;
            else if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_LS)
                retval = await lsPLCComm.SendPLCValue2PLC(value);
            //else
            //    retval = await melseqPLCComm.SendPLCValue2PLC(value);
            return retval;
        }
        ...
        public async Task<ITNTResponseArgs> SendMarkingStatus(byte status)
        {
            ...
            if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_AB)
                ...
                retval = await lsPLCComm.SendMarkingStatus(status);
            //else
            //    retval.execResult = PLC_ERR_UNSUPPORTED_PLC;
            //retval = await melseqPLCComm.SendMarkingStatus(status);
            return retval;
        ...
        public async Task<ITNTResponseArgs> SendVisionResult(string result)
        {
            ...
            if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_AB)
                ...
                retval = await lsPLCComm.SendVisionResult(result);
            //else
            //    retval = await melseqPLCComm.SendVisionResult(result);
            return retval;
        }
        ...
        public async Task<ITNTResponseArgs> SendScanComplete()
        {
            ...
            if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_AB)
                ...
                retval = await lsPLCComm.SendScanComplete();
            //else
            //    retval = await melseqPLCComm.SendVisionResult(result);
            return retval;
        }
        ...
        public async Task<ITNTResponseArgs> GetLinkAsync()
        {
            ...
            if (plcType == PLC_DEVICE_TYPE.PLC_DEVICE_TYPE_AB)
                ...
                retval = await lsPLCComm.GetLinkAsync();
            //else
            //    retval = await melseqPLCComm.SendVisionResult(result);
            return retval;
        }
