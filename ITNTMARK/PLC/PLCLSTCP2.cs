...
namespace ITNTMARK
{
    public class PLCLSTCP2
    {
        ...
        string sErrorDevice = "PLC";


        public PLCLSTCP2(PLCDataArrivedCallbackHandler callback)
        {
        ...
        public ITNTResponseArgs AnalizeRecvData(byte[] recv, int size)
        {
            ...
            catch (Exception ex)
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, string.Format("EXCEPTION - CODE = {0:X}, MSG = {1}", ex.HResult, ex.Message), Thread.CurrentThread.ManagedThreadId);
                retval.execResult = ex.HResult;
                retval.errorInfo.sErrorDevice = "AnalizeRecvData";
                retval.errorInfo.sErrorDevMsg = "PLC " + " EXCEPTION = " + ex.Message;
                return retval;
            }
            //ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "END");
        }
        ...
        private ITNTResponseArgs RecvResponseMsg(byte RWFlag, int loglevel, int timeout = 2)
        {
            ...
            try
            {
                ...
                recvLeng = workSocket.Receive(state.buffer);
                if (recvLeng <= 0)
                {
                    retval.execResult = -5;
                    return retval;
        ...
        private int Send(byte[] sdata, int leng)
        {
            ...
            try
            {
                ...
                workSocket.Send(sdata, leng, SocketFlags.None);
                //if(workSocket != null)
                return sentleng;
            }
            catch (Exception ex)
