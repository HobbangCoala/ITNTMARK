...
namespace ITNTMARK
{
    internal class PLCLSTCP3
    {
        PLCDataArrivedCallbackHandler callbackHandler;
        //PLCConnectionStatusChangedEventHandler PLCStatusChagnedEventFunc;
        //public event PLCConnectionStatusChangedEventHandler PLCStatusChagnedEventFunc;
        ...
        csConnStatus connstatusBackup = csConnStatus.Closed;
        ConnectionStatusChangedEventArgs args = new ConnectionStatusChangedEventArgs();


        public PLCLSTCP3(PLCDataArrivedCallbackHandler callback, PLCConnectionStatusChangedEventHandler statusCallback)
        {
            callbackHandler = callback;
            //PLCStatusChagnedEventFunc = statusCallback;
        ...
        public async Task<int> OpenPLCAsync(short timeout)
        {
            ...
            try
            {
                ...
                if (isConnected == true)
                {
                    ...
                    //_plcStatusThread = new Thread(PLCStatusThread);
                    //_plcStatusThread.Start();
                    args.newstatus = csConnStatus.Connected;
                    if (connstatusBackup != args.newstatus)
                    {
                        args.oldstatus = connstatusBackup;
                        //PLCStatusChagnedEventFunc?.Invoke(args);
                        connstatusBackup = args.newstatus;
                    }
                ...
                else
                {
                    ...
                    args.newstatus = csConnStatus.Closed;
        ...
        //                // Create a TCP/IP socket.  
        //                state.workSocket = new Socket(ipAddress.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
        //                state.workSocket.Connect(remoteEP);

        //                if(state.workSocket.Connected == true)
        //                {
        //                    SetWorkSocket(state.workSocket);
        //                }
        ...
        //                //else
        //                //{
        //                //    ITNTMESLog.Instance.Trace(0, "{0}::{1}()  {2}", className, funcName, "NOT CONNECTED");
        //                //    return -1;
        //                //}
        //                return 0;
        //#endif
        ...
        private async void ConnectComplete(object sender, SocketAsyncEventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    Util.GetPrivateProfileValue("PLCCOMM", "SERVERIP", "192.168.1.2", ref serverIP, ITNTCOMMON.Constants.PARAMS_INI_FILE);
                    ...
                    arg.UserToken = state.workSocket;
                    arg.Completed += ConnectComplete;
                    state.workSocket.ConnectAsync(arg);

                    ////args.newstatus = csConnStatus.Connecting;
                    ////if (connstatusBackup != args.newstatus)
                    ////{
                    ////    args.oldstatus = connstatusBackup;
                    ////    ConnectionStatusChangedEventFunc?.Invoke(this, args);
                    ////    connstatusBackup = args.newstatus;
                    ////}

                    //// Create a TCP/IP socket.  
                    //ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "beginConnect again", Thread.CurrentThread.ManagedThreadId);
            ...
            try
            {
                ...
                if (e.BytesTransferred <= 0)
                {
                    if ((workSocket != null) && workSocket.Connected)
                    {
                        ...
                        ITNTTraceLog.Instance.Trace(1, "{0}:{3:D4}:{1}()  {2}", className, funcName, "IP = " + serverIP + ", PORT = " + sPort, Thread.CurrentThread.ManagedThreadId);
            ...
            try
            {
                if (RWFlag == PLC_MODE_WRITE)
                {
                    if (sArg.dataSize <= 0)
                    {
                        ...
                        retval.sErrorMessage = "ERROR : SIZE <= 0";
                        return retval;
                    }
                ...
                if (retval.execResult <= 0)
                {
                    ...
                    retval.sErrorMessage = "Send FAILURE";
                    return retval;
                }
        ...
        //protected virtual void OnPLCConnectionStatusChangedFunc(ConnectionStatusChangedEventArgs e)
        //{
        //    //PLCDataArrivedEventHandler handler = PLCDataArrivedEventFunc;
        //    //if (handler != null)
        //    //    handler(this, e);
        //    PLCStatusChagnedEventFunc?.Invoke(e);
        //}
