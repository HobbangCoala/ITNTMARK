...
namespace ITNTMARK
{
    public class PLCMELSEQSerial// : Serial
    {
        ...
        public const string PLC_PC_ERROR_ADDRESS_D200 = "D0700";              // PC 정보
        ...
        public PLCMELSEQSerial(PLCDataArrivedCallbackHandler callback)
        {
            ...
            plcLogFlag = (int)Util.GetPrivateProfileValueUINT("PLCCOMM", "LOGLEVEL", 0, Constants.PARAMS_INI_FILE);
        }
        ...
        public async Task<ITNTResponseArgs> OpenPLCAsync(string port, int baud, int databits)
        {
            ...
            try
            {
                ...
                retval = await OpenPortAsync(port, baud, databits, Parity.None, StopBits.One);
        ...
        public ITNTResponseArgs OpenPLC(string port, int baud, int databits)
        {
            string className = MethodBase.GetCurrentMethod().DeclaringType.Name;
            string funcName = MethodBase.GetCurrentMethod().Name;
            ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "START", Thread.CurrentThread.ManagedThreadId);
            ...
            try
            {
                ...
                if ((Port != null) && (Port.IsOpen))
                    return retval;
                retval = OpenPort(port, baud, databits, Parity.None, StopBits.One);
                if (retval.execResult == 0)
                {
                    //statusThread = new Thread(ThreadStatusCheck);
                    //statusThread.Start();
        ...
        protected ITNTResponseArgs OpenPort(string port, int baud, int databits, Parity parity, StopBits stopbit, int readtimeout = -1, int writetimeout = -1)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
        ...
        protected async Task<ITNTResponseArgs> OpenPortAsync(string port, int baud, int databits, Parity parity, StopBits stopbit, int readtimeout = -1, int writetimeout = -1)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
        ...
        private int RecvResponseMsg(byte RWFlag, ref byte[] respMsg, ref int respSize, int loglevel, int timeout = 2)
        {
            ...
            if (RecvFlag == (byte)RECVFLAG.RECVFLAG_RECV_REQ)
            {
                ...
                return (int)COMMUNICATIONERROR.ERR_RECV_ACK_TIMEOUT;
            }
        ...
        public async Task<int> SendCommandMsgAsync(byte RWFlag, string address, byte[] msg, int size, int loglevel, int timeout=2)
        {
            ...
            try
            {
                ...
                if (Port.IsOpen == false)
                {
                    ...
                    retval = (int)COMMUNICATIONERROR.ERR_PORT_NOT_OPENED;
                    return retval;
                }
        ...
        {
            string className = "PLCMELSEQSerial";
...
#else
            retval = await ExecuteCommandMsgAsync(PLC_MODE_READ, PLC_ADDRESS_D100, recv, PLC_D_LEN_01, loglevel, 2);
            if (retval.execResult == (int)COMMUNICATIONERROR.ERR_COMMAND_BUSY)
            {
                retval = await ExecuteCommandMsgAsync(PLC_MODE_READ, PLC_ADDRESS_D100, recv, PLC_D_LEN_01, loglevel, 2);
                if (retval.execResult == (int)COMMUNICATIONERROR.ERR_COMMAND_BUSY)
