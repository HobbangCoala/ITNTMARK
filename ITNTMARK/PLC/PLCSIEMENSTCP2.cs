...
namespace ITNTMARK.PLC
{
    internal class PLCSIEMENSTCP2
    {
        ...
        public const string PLC_ADDRESS_SEQ = "300";                // Seq no
        public const string PLC_ADDRESS_CAR = "400";                // Car type
        ...
        public const string PLC_ADDRESS_D120 = "120";               // PLC -> PC(수동=1, 자동=0)

        //---------------------------------------------------------------------------------------------------
        // System에서 사용될 PC --> PLC간 데이터 전송에 사용되는 D 영역 설정
        //---------------------------------------------------------------------------------------------------
        public const string PLC_ADDRESS_D200 = "600";               // PC 정보
        ...
        public const string PLC_ADDRESS_D230_MARK = "600";                  // PC 정보 (각인상태) 1 - 각인중, 2 - 각인완료
        public const string PLC_ADDRESS_D240_VISION = "600";                // vision result 1 - OK, 2 - NG, 4 - Shalow
        public const string PLC_ADDRESS_D610_LINK_COMMAND = "610";          // Linking Address : 1 - Linking, 2 - Unlinking
        public const string PLC_ADDRESS_D520_LINK_STATUS = "520";           // Linking Status Address : 1 - Linking, 2 - Unlinking
        ...
        public PLCSIEMENSTCP2(PLCDataArrivedCallbackHandler callback)
        {
        ...
        public async Task<int> OpenPLCAsync(short timeout)
        {
            ...
            try
            {
                ...
                return 0;
#else
                Util.GetPrivateProfileValue("PLCCOMM", "SERVERIP", "192.168.1.2", ref sIPAddr, ITNTCOMMON.Constants.PARAMS_INI_FILE);
                Util.GetPrivateProfileValue("PLCCOMM", "SERVERPORT", "2004", ref sPort, ITNTCOMMON.Constants.PARAMS_INI_FILE);

                Util.GetPrivateProfileValue("PLCCOMM", "RACKNO", "192.168.1.2", ref sRack, ITNTCOMMON.Constants.PARAMS_INI_FILE);
                Util.GetPrivateProfileValue("PLCCOMM", "SLOTNO", "2004", ref sSlot, ITNTCOMMON.Constants.PARAMS_INI_FILE);

                int.TryParse(sPort, out iPort);
                short.TryParse(sRack, out wRack);
                short.TryParse(sSlot, out wRack);

                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "IP = " + sIPAddr + ", PORT = " + sPort, Thread.CurrentThread.ManagedThreadId);

                //if (simensPLC == null)
                //{
                //    simensPLC = new Plc(CpuType.S7200, sIPAddr, iPort, wRack, wSlot);
                //    await simensPLC.OpenAsync();
                //    sw.Start();
                //    while (sw.Elapsed < TimeSpan.FromSeconds(timeout))
                //    {
                //        if (simensPLC.IsConnected == true)
                //            break;
                ...
                //    //if(_plcStatusThread.ThreadState)
                //    simensPLC.Close();
                //    //simensPLC.
                //}
        ...
        public int OpenPLC(short timeout)
        {
            string className = "PLCSIEMENSTCP";
            string funcName = "OpenPLCAsync";
            int retval = 0;
...
#else
                Util.GetPrivateProfileValue("PLCCOMM", "SERVERIP", "192.168.1.2", ref sIPAddr, ITNTCOMMON.Constants.PARAMS_INI_FILE);
                Util.GetPrivateProfileValue("PLCCOMM", "SERVERPORT", "125", ref sPort, ITNTCOMMON.Constants.PARAMS_INI_FILE);
        ...
        public async Task<ITNTResponseArgs> ReadPLCAsync(ITNTSendArgs sendArg, CancellationToken cancelToken = default)
        {
            ...
            try
            {
                ...
                await Task.Delay(500);
#else
                retval = await ExecuteCommandFuncAsync(PLC_MODE_READ, sendArg, sendArg.loglevel);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> ReadPLCAsync(string strAdd)
        {
            ...
            try
            {
                ...
                retval = await ExecuteCommandFuncAsync(PLC_MODE_READ, sArg, 2);
#endif
            }
            catch (Exception ex)
            {
...
#else
                sArg.AddrString = strAdd;
                retval = ExecuteCommandFunc(PLC_MODE_READ, sArg, 2);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> WritePLCAsync(ITNTSendArgs sendArg)
        {
            ...
            try
            {
                ...
                await Task.Delay(500);
#else
                retval = await ExecuteCommandFuncAsync(PLC_MODE_WRITE, sendArg, sendArg.loglevel);
#endif
            }
            catch (Exception ex)
            {
...
#else
                retval = await ExecuteCommandFuncAsync(PLC_MODE_WRITE, sArg, 0, 2);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> SendPCError2PLC(string plcvalue)
        {
            ...
            retval.recvSize = value.Length;
                ...
                retval = await ExecuteCommandFuncAsync(PLC_MODE_WRITE, sArg, 3, 1);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> ReadPLCCarType()
        {
            ...
            retval.recvSize = value.Length;
                ...
                retval = await ExecuteCommandFuncAsync(PLC_MODE_READ, sArg, 3, 2);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> SendSignal(byte signal)
        {
            ...
            retval.recvSize = value.Length;
                ...
                retval = await ExecuteCommandFuncAsync(PLC_MODE_WRITE, sArg, 3, 2);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> SendVisionResult(string result)
        {
            ...
            try
            {
                ...
                if (result == "O")
                    sArg.sendBuffer[0] = 16;
                else
                    sArg.sendBuffer[0] = 32;
                sArg.dataSize = 1;
        ...
        public async Task<ITNTResponseArgs> GetLinkAsync()
        {
            ITNTResponseArgs retval = new ITNTResponseArgs(256);
            ITNTSendArgs sArg = new ITNTSendArgs();
            //byte[] recv = new byte[256];
            ...
            retval.recvSize = value.Length;
#else
                retval = await ExecuteCommandFuncAsync(PLC_MODE_READ, sArg, sArg.loglevel, 2);
#endif
            }
            catch (Exception ex)
            {
        ...
        public ITNTResponseArgs AnalizeRecvData(byte[] recv, int size)
        {
            ...
            try
            {
                ...
                {
                    retval.execResult = (int)TCPCOMMERROR.ERR_RECV_DATA_NONE;
                    return retval;
                }
        ...
        public async Task<int> SendCommandMsgAsync(byte RWFlag, ITNTSendArgs sArg, int loglevel, int timeout = 2)
        {
            string className = "PLCSIEMENSTCP";// MethodBase.GetCurrentMethod().DeclaringType.Name;
            ...
            try
            {
                if (RWFlag == PLC_MODE_WRITE)
                {
                    if (sArg.dataSize <= 0)
                    {
                        ...
                        retval = (int)TCPCOMMERROR.ERR_BUFFER_SIZE_ERROR;
                        return retval;
                    }
                ...
                //    retval = (int)TCPCOMMERROR.ERR_COMMAND_BUSY;
                //    return retval;
                //}
        ...
        private async Task<ITNTResponseArgs> RecvResponseMsgAsync(byte RWFlag, int loglevel, int timeout = 2)
        {
            ...
            try
            {
                ...
                if (RecvFlag != (byte)RECVFLAG.RECVFLAG_RECV_END)
                {
                    ...
                    retval.execResult = (int)TCPCOMMERROR.ERR_RECV_RESPONSE_TIMEOUT;
                ...
                //    retval.execResult = (int)TCPCOMMERROR.ERR_RECV_NAK;
                //    RecvFlag = (byte)RECVFLAG.RECVFLAG_RECV_ERR;
                //    return retval;
                //}
        ...
        //    if (size <= 0)
        //    {
        //        return (int)TCPCOMMERROR.ERR_BUFFER_SIZE_ERROR;
        //    }

        //    if (Port.IsOpen == false)
        //    {
        //        return (int)TCPCOMMERROR.ERR_PORT_NOT_OPENED;
        //    }
        ...
        //            if (retval == 0)
        //                break;
        //            else if (retval == (int)TCPCOMMERROR.ERR_RECV_NAK)
        //            {
        //                lock (bufferLock)
        //                {
        //                    cb.Clear();
        ...
        //        //if (size <= 0)
        //        //{
        //        //    retval.execResult = (int)TCPCOMMERROR.ERR_BUFFER_SIZE_ERROR;
        //        //    return retval;
        //        //}
        ...
        //        if (doingCmdFlag)
        //        {
        //            retval.execResult = (int)TCPCOMMERROR.ERR_COMMAND_BUSY;
        //            ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "ERROR : BUSY");
        //            return retval;
        //        }
        ...
        //            if (retval.execResult == 0)
        //                break;
        //            //else if (retval.execResult == (int)TCPCOMMERROR.ERR_RECV_NAK)
        //            //{
        //            //    lock (bufferLock)
        //            //    {
        //            //        cb.Clear();
        ...
        //                //doingCmdFlag = false;
        //                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, string.Format("RecvResponseMsg ERROR : {0} / {1}", retval, retrycount));
        //                if (retval.execResult == (int)TCPCOMMERROR.ERR_RECV_NAK)
        //                {
        ...
        public async Task<ITNTResponseArgs> ExecuteCommandMsgAsync(byte RWFlag, ITNTSendArgs sArg, int loglevel, int timeout = 2)
        {
            ...
            try
            {
                ...
                {
                    InitializeExecuteCommand();
                    retval.execResult = await SendCommandMsgAsync(RWFlag, sArg, loglevel, timeout);
                    if (retval.execResult <= 0)
                    {
        ...
        public async Task<ITNTResponseArgs> ExecuteCommandFuncAsync(byte RWFlag, ITNTSendArgs sArg, int loglevel, int timeout = 2)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs(256);
        ...
        public ITNTResponseArgs ExecuteCommandFunc(byte RWFlag, ITNTSendArgs sArg, int loglevel, int timeout = 2)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs(256);

            try
            {
                retval = ExecuteCommandMsg(PLC_MODE_READ, sArg, sArg.loglevel, 2);
                if (retval.execResult == (int)TCPCOMMERROR.ERR_COMMAND_BUSY)
                {
                    retval = ExecuteCommandMsg(PLC_MODE_READ, sArg, sArg.loglevel, 2);
                    if (retval.execResult == (int)TCPCOMMERROR.ERR_COMMAND_BUSY)
        ...
        //        Util.GetPrivateProfileValue("PLCCOMM", "SERVERIP", "192.168.0.50", ref value, ITNTCOMMON.Constants.PARAMS_INI_FILE);
        //        serverIP = value;
        //        Util.GetPrivateProfileValue("PLCCOMM", "SERVERPORT", "256000", ref value, ITNTCOMMON.Constants.PARAMS_INI_FILE);
        ...
        //    {
        //        //bconnected = false;
        //        ITNTMESLog.Instance.Trace(0, "{0}::{1}()  {2}", className, funcName, string.Format("EXCEPTION - CODE = {0:X}, MSG = {1}", ex.HResult, ex.Message));
        //    }
        ...
        private async void ConnectComplete(object sender, SocketAsyncEventArgs e)
        {
            ...
            try
            {
                ...
                if (isConnected)
                {
                    ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MESClient", "connectComplete", "isConnected = true", Thread.CurrentThread.ManagedThreadId);
            ...
            try
            {
                ...
                if (e.BytesTransferred <= 0)
                {
                    if ((workSocket != null) && workSocket.Connected)
                    {
                        ...
                        Util.GetPrivateProfileValue("PLCCOMM", "SERVERIP", "192.168.1.2", ref IPAddr, ITNTCOMMON.Constants.PARAMS_INI_FILE);
                        Util.GetPrivateProfileValue("PLCCOMM", "SERVERPORT", "2004", ref Port, ITNTCOMMON.Constants.PARAMS_INI_FILE);
        ...
        public async Task<ITNTResponseArgs> SetCommSettingTCP(string IP, int Port)
        {
            ...
            {
                if (IP.Length > 0)
                    Util.WritePrivateProfileValue("PLCCOMM", "SERVERIP", IP, ITNTCOMMON.Constants.PARAMS_INI_FILE);
                if (Port > 0)
                    Util.WritePrivateProfileValue("PLCCOMM", "SERVERPORT", Port.ToString(), ITNTCOMMON.Constants.PARAMS_INI_FILE);
            }
            catch (Exception ex)
            {
                retval.execResult = ex.HResult;
        ...
        public ITNTResponseArgs GetCommSettingTCP(ref string IP, ref int Port)
        {
            ...
            try
            {
                Util.GetPrivateProfileValue("PLCCOMM", "SERVERIP", "", ref IP, ITNTCOMMON.Constants.PARAMS_INI_FILE);
                Util.GetPrivateProfileValue("PLCCOMM", "SERVERPORT", "", ref value, ITNTCOMMON.Constants.PARAMS_INI_FILE);
