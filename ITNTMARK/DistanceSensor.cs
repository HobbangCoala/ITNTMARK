...
namespace ITNTMARK
{
    ...
    class DistanceSensor
    {
        ...
        private readonly SemaphoreSlim _cmdSemaphore = new SemaphoreSlim(1, 1);

        //string sDeviceCode = "8";
        //string sDeviceName = "DISPLACEMENT";


        public DistanceSensor(ConnectionChangedEventHandler callback)//distanceSensorDataArrivedCallbackHandler DataCallback)//, ClientConnectionHandler ConnectCallback)
        {
...
//                Console.WriteLine(ex.ToString());
        ...
        public void ReceiveCommData()
        {
            ...
            try
            {
                lock (lockbuff)
                {
                    int totalAvailable = rb.GetSize();
                    if (totalAvailable <= 0) return;

                    byte[] look = new byte[totalAvailable];
                    rb.Look(ref look, totalAvailable);
        ...
        public async Task<ITNTResponseArgs> ExecuteCommandMsgAsync(string cmd, byte[] msg, int size, int retrycnt, int loglevel, int timeout = 1000, int waitTimeout=3000)
        {
            ...
            try
            {
                ...
                bool acquired = await _cmdSemaphore.WaitAsync(waitTimeout);
                if (!acquired)
                {
                ...
                //sw.Start();
                //while (sw.Elapsed < TimeSpan.FromMilliseconds(waitTimeout))
                //{
                //    if (!doingCmdFlag)
                //        break;
        ...
        private async Task<ITNTResponseArgs> RecvResponseMsgAsync(string cmd, int loglevel, int timeout = 1000)
        {
            string className = "DistanceSensor";
