...
namespace ITNTCOMMON
{
    class ITNTErrorLog
    {
        private static readonly Lazy<ITNTErrorLog>
            lazy =
            new Lazy<ITNTErrorLog>
            (() => new ITNTErrorLog());

        public static ITNTErrorLog Instance { get { return lazy.Value; } }
        ...
        private ITNTErrorLog()
        {
            OpenFile();
            string val = "";
            Util.GetPrivateProfileValue("OPTION", "ERRLOGLEVEL", "1", ref val, Constants.PARAMS_INI_FILE);
            logLevel = Convert.ToInt32(val);
        }

        ITNTErrorLog(string strFileName)
        {
        ...
        private bool OpenFile()
        {
            ...
            DateTime dt = DateTime.Now;
            createTime = dt;
            curDir = curDir + "ERRLOG\\" + dt.ToString("yyyy");
