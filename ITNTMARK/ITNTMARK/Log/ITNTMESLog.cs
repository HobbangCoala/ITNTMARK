...
namespace ITNTCOMMON
{
    class ITNTMESLog
    {
        private static readonly Lazy<ITNTMESLog>
    lazy =
    new Lazy<ITNTMESLog>
    (() => new ITNTMESLog());

        public static ITNTMESLog Instance { get { return lazy.Value; } }
        ...
        private ITNTMESLog()
        {
            OpenFile();
            string val = "";
            Util.GetPrivateProfileValue("OPTION", "MESLOGLEVEL", "1", ref val, Constants.PARAMS_INI_FILE);
            logLevel = Convert.ToInt32(val);
        }

        ITNTMESLog(string strFileName)
        {
        ...
        private bool OpenFile()
        {
            ...
            try
            {
                ...
                logfilename = curDir + "\\MES_" + dt.ToString("dd") + ".dat";
