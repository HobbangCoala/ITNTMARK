...
namespace ITNTMARK
{
    ...
    internal class PLCABTCP7
    {
        ...
        volatile bool bOpend = false;  // [수정] volatile 추가
        bool bTimerReadPLC = false;
        private int errorCount = 0;
        string statusAddr = "";

        private readonly object cmdLock = new object();
        private readonly object timerLock = new object();  // [추가] 타이머 Lock

        volatile bool DoingPLCStatusThread = false;  // [수정] volatile 추가
        Thread _plcStatusThread = null;
        CancellationTokenSource tokenSource = new CancellationTokenSource();
        csConnStatus connState = csConnStatus.Closed;

        private bool heartbeatTimerInitialized = false;  // [추가] 타이머 초기화 플래그

        public PLCABTCP7(PLCDataArrivedCallbackHandler callback, PLCConnectionStatusChangedEventHandler connectFunc)
        {
        ...
        // [추가] Heartbeat 타이머 초기화 메서드 - 중복 등록 방지
        private void InitializeHeartbeatTimer()
        {
            lock (timerLock)
            {
                // 기존 타이머 정리
                if (heartbeatTimerInitialized)
                {
                    heartbeatTimer.Stop();
                    heartbeatTimer.Elapsed -= HeartbeatTimerFunc;
                }

                heartbeatTimer.Interval = 1000;
                heartbeatTimer.Elapsed += new ElapsedEventHandler(HeartbeatTimerFunc);
                heartbeatTimerInitialized = true;
            }
        }
        ...
        public ITNTResponseArgs OpenPLC()
        {
            ...
            try
            {
                ...
                if (retval.execResult == 0)
                {
                    ...
                    _plcStatusThread.IsBackground = true;  // [추가] 백그라운드 스레드 설정
                    _plcStatusThread.Start();  // [수정] 파라미터는 멤버 변수 사용으로 변경
                ...
                else
                {
                    ...
                    // [수정] 타이머는 초기화만 하고 시작하지 않음 (연결 성공 시 시작)
                    InitializeHeartbeatTimer();

                    DoingPLCStatusThread = true;
                    _plcStatusThread = new Thread(PLCStatusThread);
                    _plcStatusThread.IsBackground = true;
        ...
        public int ClosePLCPart()
        {
            ...
            try
            {
                ...
                {
                    try { ABPLCControl.CloseConnection(); } catch { }
                }
                bOpend = false;
        ...
        // [추가] Dispose 패턴 - 프로그램 종료 시 호출
        public int ClosePLC()
        {
            string className = "PLCABTCP2";
            string funcName = "Dispose";
            int retval = 0;
            ...
            try
            {
                ...
                // Thread가 종료될 때까지 대기 (최대 3초)
                if (_plcStatusThread != null && _plcStatusThread.IsAlive)
                {
                    if (!_plcStatusThread.Join(3000))
                    {
                        ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, 
                            "WARNING: PLCStatusThread did not terminate within timeout", Thread.CurrentThread.ManagedThreadId);
                    }
                }

                // Heartbeat 타이머 정지 및 정리
                StopHeartbeatTimer();
                lock (timerLock)
                {
                    if (heartbeatTimerInitialized)
                    {
                        heartbeatTimer.Elapsed -= HeartbeatTimerFunc;
                        heartbeatTimerInitialized = false;
                    }
                }
            ...
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  EXCEPTION = {2}", className, funcName, ex.Message, Thread.CurrentThread.ManagedThreadId);
                retval = ex.HResult;
            }
            return retval;
        }
        ...
        private ITNTResponseArgs ReadPLC(string addr, /*ref string msg, ref int leng, */int level, CancellationToken token = default)
        {
            ...
            {
                // [수정] Lock 외부에서 ClosePLC 호출하도록 변경
                // bOpend를 먼저 false로 설정하여 추가 통신 시도 방지
                bOpend = false;
        ...
        private ITNTResponseArgs ReadPLC4Test(string addr, int level, CancellationToken token = default)
        {
            string className = "PLCABTCP2";
        ...
        // [수정] PLCStatusThread - 파라미터 대신 멤버 변수 사용
                    ...
                    if ((ABPLCControl == null) || (bOpend == false))
                    {
                        ...
                        //lock (cmdLock)
                        {
                            ...
                            if (retval.execResult == 0)
                            {
                                ...
                                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, 
                                    "RECONNECT SUCCESS", Thread.CurrentThread.ManagedThreadId);
                            }
                            else
                            {
                                bOpend = false;
