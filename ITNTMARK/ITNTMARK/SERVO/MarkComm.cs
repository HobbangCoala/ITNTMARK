...
namespace ITNTMARK
{
    public class MarkComm// : Serial
    {
        ...
        //                //ITNTTraceLog.Instance.TraceHex(0, "MarkComm::ReadPortThread()  RECV DATA : ", readsize, ref readBuffer, "");
        //                if (readsize > 0)
        //                {
        //                    lock (bufferLock)
        //                    {
        ...
        protected void ReceiveCommData()
        {
            ...
            try
            {
                while ((count = cb.GetSize()) > 7)
                {
                        ...
                        if (idxSOH >= 0)
                        {
                            ...
                            if (idxCR > idxSOH)
                            {
                                ...
                                if ((recv[idxSOH + 4] >= '0') && (recv[idxSOH + 4] <= '9'))
                                {
                                    ...
                                    if (size < 18)
                                    {
                                        ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, string.Format("SIZE={0} SO={1}", size, idxSOH), Thread.CurrentThread.ManagedThreadId);
                                        return;
                                    }
                    ...
                    {
                        Debug.WriteLine("!!!!");
                        ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "Not Contain(SOH)", Thread.CurrentThread.ManagedThreadId);
                        return;
                    }
                    recv.Initialize();
                }
        ...
        //protected void ReceiveCommData2()
        //{
        //    string className = "MarkComm";
        ...
        public async Task<int> SendCommandMsg(int loglevel, byte type, byte[]sendData, int sendLength)
        {
            ...
            try
            {
                //if(bReadyError)     //D J C X K R W M D
                //{
                //    //if ((type == (byte)'D') || (type == (byte)'J') || (type == (byte)'C') ||
                //    //(type == (byte)'X') || (type == (byte)'K') || (type == (byte)'R') ||
                //    //(type == (byte)'W') || (type == (byte)'M'))
                ...
                //    {
                //        ITNTTraceLog.Instance.Trace(loglevel, "{0}::{1}()  {2}", className, funcName, "COMMUNICATIONERROR.ERR_NOT_READY");
                //        return (int)COMMUNICATIONERROR.ERR_NOT_READY;
                //    }
                //}
        ...
        /// <returns></returns>
        public async Task<ITNTResponseArgs> RecvResponseMsg(int loglevel, byte type, int timeout1 = 2, int timeout2 = 6)
        {
            //int retval = 0;
            string className = "MarkComm";// MethodBase.GetCurrentMethod().DeclaringType.Name;
            ...
            try
            {
                ...
                //    (type == (byte)'J') || (type == (byte)'C') || (type == (byte)'M') ||
                //    (type == (byte)'K') || (type == (byte)'U') || (type == (byte)'h') ||
                //    (type == (byte)'j') || (type == (byte)'k') || (bReadyError)) //R r H J C M K U h j k )
                //{
                //    timeout = timeout2;
                //    endStatus = (byte)RECVFLAG.RECVFLAG_RECV_S38;
                //}
                //else
                {
                    timeout = timeout1;
                    endStatus = (byte)RECVFLAG.RECVFLAG_RECV_END;
                }

                sw.Start();
                while (sw.Elapsed < TimeSpan.FromSeconds(timeout))
                {
                    if (RecvFlag == endStatus)
                        break;
        ...
        public async Task<ITNTResponseArgs> ExecuteCommandMsg(int loglevel, int timeoutsec, byte type, byte[] sendData, int sendLength)
        {
            ...
            try
            {
                ...
                //    (type == (byte)'j') || (type == (byte)'k') || (bReadyError)) //R r H J C M K U h j k )
                //{
                //    ITNTTraceLog.Instance.Trace(loglevel, "{0}:{3:D4}:{1}()  {2}", className, funcName, string.Format("{0} - ERR_NOT_READY", type));
                //    retval.execResult = (int)COMMUNICATIONERROR.ERR_NOT_READY;
                //    return retval;
                //}
                ...
                //lock (cmdLock)
                {
                    for (retrycount = 0; retrycount < 3; retrycount++)
                    {
                        ...
                        retval = await RecvResponseMsg(loglevel, type, timeoutsec);
                        if (retval.execResult == 0)
                            break;
...
//                if (retval.execResult != 0)
//                {
//                    ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "InitializeController", string.Format("LoadSpeed-F ERROR : {0}", retval.execResult));
//                    return retval;
//                }
...
//                if (retval.execResult != 0)
//                {
//                    ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "InitializeController", string.Format("LoadSpeed-l ERROR : {0}", retval.execResult));
//                    return retval;
//                }
...
//                if(retval.execResult != 0)
//                {
//                    ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "InitializeController", string.Format("TestSolFet ERROR : {0}", retval.execResult));
//                    return retval;
//                }
...
//                if (retval.execResult != 0)
//                {
//                    ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "InitializeController", string.Format("GoHome ERROR : {0}", retval.execResult));
//                    return retval;
//                }
//#endif
//            }
...
//        }
        ...
        public async Task<ITNTResponseArgs> InitializeController()
        {
            ...
            try
            {
                ...
                //short scanmaxY = (short)Util.GetPrivateProfileValueUINT("CONFIG", "MAX_U", 180, Constants.SCANNER_INI_FILE);
                ...
                //retval = await GoHome_Z((short)home_u);
                //if (retval.execResult != 0)
                //{
                //    ITNTTraceLog.Instance.Trace(0, "{0}::{1}()  {2}", "MarkComm", "InitializeController", string.Format("GoHome ERROR : {0}", retval.execResult));
                //    return retval;
                //}
#endif
            }
...
//            int use2 = (int)Util.GetPrivateProfileValueUINT("MARKER", "USE", 0, "TEST.ini");
        ...
        public async Task<ITNTResponseArgs> MoveHead(short xval, short yval)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
            byte[] sdata = new byte[64];
            string sstrring = "";
            try
            {
                ...
                sstrring = xval.ToString("X4") + yval.ToString("X4");
                sdata = Encoding.UTF8.GetBytes(sstrring);
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(0, 5, (byte)'J', sdata, sdata.Length);
#endif
        ...
        public async Task<ITNTResponseArgs> MoveScan2Home(short homeposition)
        {
            ...
            try
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "MoveScan2Home", "START", Thread.CurrentThread.ManagedThreadId);
                sstrring = homeposition.ToString("X4");
                sdata = Encoding.UTF8.GetBytes(sstrring);
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(0, 2, (byte)'h', sdata, sdata.Length);
#endif
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "MoveScan2Home", "END", Thread.CurrentThread.ManagedThreadId);
                return retval;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> ScanProfile(short length)
        {
            ...
            try
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "ScanProfile", "START", Thread.CurrentThread.ManagedThreadId);
                sstrring = length.ToString("X4");
                sdata = Encoding.UTF8.GetBytes(sstrring);
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(0, 2, (byte)'U', sdata, sdata.Length);
#endif
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "ScanProfile", "END", Thread.CurrentThread.ManagedThreadId);
                return retval;
            }
            catch (Exception ex)
            {
...
//            try
//            {
//                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "MoveScanProfile", "START", Thread.CurrentThread.ManagedThreadId);
//                sstrring = ((short)(position * steplength)).ToString("X4");
//                sdata = Encoding.UTF8.GetBytes(sstrring);
//#if TEST_DEBUG_MARK
//#else
//                retval = await ExecuteCommandMsg(0, 5, (byte)'k', sdata, sdata.Length);
//#endif
//                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "MoveScanProfile", "END", Thread.CurrentThread.ManagedThreadId);
//                return retval;
//            }
//            catch (Exception ex)
//            {
...
//        }
...
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(0, 2, (byte)'k', sdata, sdata.Length);
#endif
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "MoveScanProfile", "END", Thread.CurrentThread.ManagedThreadId);
                return retval;
            }
        ...
        public async Task<ITNTResponseArgs> TestSolFet(short Fet, bool Sol)
        {
            ...
            try
            {
				...
				retval = await ExecuteCommandMsg(0, 5, (byte)'O', sdata, sdata.Length);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> GoHome(int posX, int posY)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
            byte[] sdata = new byte[64];
            string sendstring = "";
        ...
        public async Task<ITNTResponseArgs> GoHomeAll(short posX, short posY, short posU)
        {
            ...
            try
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "GoHomeAll", "START", Thread.CurrentThread.ManagedThreadId);
                sendstring = posX.ToString("X4") + posY.ToString("X4") + posU.ToString("X4");
                sdata = Encoding.UTF8.GetBytes(sendstring);
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(1, 5, (byte)'E', sdata, sdata.Length);
#endif
            }
            catch (Exception ex)
            {
            ...
            ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "GoHomeAll", "END", Thread.CurrentThread.ManagedThreadId);
            return retval;
        }

        public async Task<ITNTResponseArgs> GoParking(int posX, int posY)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
            byte[] sdata = new byte[64];
            string sendstring = "";
        ...
        public async Task<ITNTResponseArgs> GoPoint(int posX, int posY, int pos)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
            byte[] sdata = new byte[64];
            string sendstring = "";
...
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(0, 5, cmd, sdata, sdata.Length);
#endif
            }
            catch (Exception ex)
            {
        ...
        public async Task<ITNTResponseArgs> LoadFontData(short chidx, short ptidx, int posX, int poxY, short flag)
        {
            ITNTResponseArgs retval = new ITNTResponseArgs();
            byte[] sdata = new byte[128];
            string sendstring = "";
        ...
        public async Task<ITNTResponseArgs> dwellTimeSet(short time)
        {
            ...
            try
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "dwellTimeSet", "START", Thread.CurrentThread.ManagedThreadId);
            ...
            ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "dwellTimeSet", "END", Thread.CurrentThread.ManagedThreadId);
            return retval;
        }
        ...
        public async Task<string> GetFWVersion()
        {
            ...
            try
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "GetFWVersion", "START", Thread.CurrentThread.ManagedThreadId);
                //sendstring = time.ToString("X4");
                //sdata = Encoding.UTF8.GetBytes(sendstring);
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(0, 5, (byte)'V', sdata, 0);
#endif
            ...
            ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "GetFWVersion", "END", Thread.CurrentThread.ManagedThreadId);
            return ver;
        }
        ...
        public async Task<ITNTResponseArgs> Test4RealMarkArea()
        {
            ...
            try
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "Test4PinMark", "START", Thread.CurrentThread.ManagedThreadId);
#if TEST_DEBUG_MARK
#else
                retval = await ExecuteCommandMsg(0, 5, (byte)'C', sdata, 0);
#endif
            }
            catch (Exception ex)
            {
            ...
            ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MarkComm", "Test4PinMark", "END", Thread.CurrentThread.ManagedThreadId);
            return retval;
        }
...
//                                SaveMarkResultData(currMarkInfo.currMarkData.mesData, 0, 0);
...
//                                Util.WritePrivateProfileValue("CURRENT", "VIN", currMarkInfo.currMarkData.mesData.vin, Constants.DATA_CUR_COMPLETE_FILE);
//                                //Util.WritePrivateProfileValue("CURRENT", "INDEX", m_CurrentMarkNum.ToString(), Constants.DATA_CUR_COMPLETE_FILE);
//                                Util.WritePrivateProfileValue("CURRENT", "SEQVIN", currMarkInfo.currMarkData.mesData.sequence + "|" + currMarkInfo.currMarkData.mesData.vin, Constants.DATA_CUR_COMPLETE_FILE);

//                                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MainWindow", "OnMarkControllerEventFunc", "Marking Complete  " + currMarkInfo.currMarkData.mesData.sequence + "-" + currMarkInfo.currMarkData.mesData.vin, Thread.CurrentThread.ManagedThreadId);
//                                //ITNTJobLog.Instance.Trace(0, "{0}::{1}()  {2}", "MainWindow", "OnMarkControllerEventFunc", "Marking Complete  " + currMarkInfo.currMarkData.mesData.sequence + "-" + currMarkInfo.currMarkData.mesData.vin);

//                                //currMarkInfo.Initialize();
//                                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MainWindow", "OnMarkControllerEventFunc", "[4] : 각인완료 종료", Thread.CurrentThread.ManagedThreadId);
...
//                }
            ...
            {
                ITNTTraceLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", "MainWindow", "OnMarkControllerEventFunc", string.Format("EXCEPTION - CODE = {0}, MSG = {1}", ex.HResult, ex.Message), Thread.CurrentThread.ManagedThreadId);
            }
        }
