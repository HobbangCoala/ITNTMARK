...
namespace ITNTMARK
{
    class MESClient3
    {
        ...
        public event MESClientStatusChangedEventHandler ServerStatusChangedEvent = null;
        ...
        csConnStatus clientconnstatus = csConnStatus.Closed;
        csConnStatus clientconnbefore = csConnStatus.Closed;
        ...
        //private byte[] szData;
        private Socket recvWorkSocket;

        csConnStatus serverconnstatus = csConnStatus.Closed;
        csConnStatus serverconnbefore = csConnStatus.Closed;
        ...
        public MESClient3()
        {
            ...
            recvWorkSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);

            //DataArrivalCallback = DataCallback;
        }

        public async Task<int> StartMES()
        {
            int retval = 0;
        ...
        public async Task<int> StartMESServer(IPAddress ipAddress, int iPort)
        {
            string className = "MESClient";
            string funcName = "StartMESServer";
            ...
            try
            {
                ...
                endPoint = new IPEndPoint(ipAddress, iPort); //엔드포인트(ip주소와 포트번호)
                ...
                if(m_ServerSocket.IsBound == false)
                {
                    ...
                    sockargs.Completed += new EventHandler<SocketAsyncEventArgs>(Accept_Completed);
                    m_ServerSocket.AcceptAsync(sockargs);
                }
        ...
        public async Task<int> OpenMESAsync(TimeSpan connectTimeout)
        {
            ...
            try
            {
                ...
                if ((GetSendWorkSocket() != null) && (GetSendWorkSocket().Connected == true))
                {
        ...
        private void Accept_Completed(object sender, SocketAsyncEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.AcceptSocket == null)
                {
                    ...
                    ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "e.AcceptSocket == NULL", Thread.CurrentThread.ManagedThreadId);
                    return;
                }
                ...
                if (socketlist.m_socket.Connected == true)
                {
                    ...
                    recvWorkSocket = socketlist.m_socket;
                    //szData = new byte[1024];
                    args.SetBuffer(recvBuffer, 0, recvBuffer.Length);
                    //args.UserToken = m_ClientSocket;
                    args.UserToken = socketlist;
                    args.Completed += new EventHandler<SocketAsyncEventArgs>(Receive_Completed);
                    socketlist.m_socket.ReceiveAsync(args);
                    ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "MES ACCEPT CONNECTED", Thread.CurrentThread.ManagedThreadId);
                }
                else
                {
                    if (m_ServerSocket == null)
            ...
            {
                ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, string.Format("EXCEPTION 1 - CODE = {0:X}, MSG = {1}", se.HResult, se.Message), Thread.CurrentThread.ManagedThreadId);
                ...
                {
                    ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, string.Format("EXCEPTION 12 - CODE = {0:X}, MSG = {1}", ex1.HResult, ex1.Message), Thread.CurrentThread.ManagedThreadId);
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, string.Format("EXCEPTION 22 - CODE = {0:X}, MSG = {1}", ex2.HResult, ex2.Message), Thread.CurrentThread.ManagedThreadId);
                }
            }
        }
        ...
        private void Receive_Completed(object sender, SocketAsyncEventArgs e)
        {
            ...
            SocketAsyncEventArgs args = new SocketAsyncEventArgs();
            SocketList socketlist = new SocketList();
            List<SocketList> list = new List<SocketList>();
            int iLeng = 0;
            string recvdata = "";
            ...
            try
            {
                ...
                //list = sender as List<SocketList>;
                //socketlist = sender as SocketList;
                ...
                if (socketlist.m_socket.Connected && e.BytesTransferred > 0)
                {
                    ...
                    iLeng = rb.Put(e.Buffer, e.BytesTransferred);

                    recvdata = Encoding.UTF8.GetString(e.Buffer, 0, iLeng);
                    ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "RECV = " + recvdata, Thread.CurrentThread.ManagedThreadId);
                    ...
                    else
                    {
                        ...
                        e.SetBuffer(recvBuffer, 0, recvBuffer.Length);
                        socketlist.m_socket.ReceiveAsync(e);
                    }
                }
                else
        ...
        public async Task<int> ConnectServer(int timeoutsec)
        {
            ...
            try
            {
                ...
                {
                    //if (!isConnected)
                    if (GetSendWorkSocket() != null)
                    {
                        if (GetSendWorkSocket().Connected)
                            break;
                    }
                ...
                if ((GetSendWorkSocket() == null) || (GetSendWorkSocket().Connected == false))
                {
            ...
            try
            {
                if (GetSendWorkSocket() != null)
                {
                    ...
                    if (GetSendWorkSocket().Connected)
                    {
                        ...
                        GetSendWorkSocket().Shutdown(SocketShutdown.Both);
                        ...
                        GetSendWorkSocket().Disconnect(false);
                        ...
                        GetSendWorkSocket().Close();
                        GetSendWorkSocket().Dispose();
        ...
        public void ReceiveCommData(byte state, int size)
        {
            string className = "MESClient3";
        ...
        private async void connectComplete(object sender, SocketAsyncEventArgs e)
        {
            ...
            try
            {
                if (e.SocketError == SocketError.Success)
                {
                    ...
                    SetSendWorkSocket((Socket)e.UserToken);
                    arg.UserToken = GetSendWorkSocket();
                    ...
                    bret = GetSendWorkSocket().ReceiveAsync(arg);
        ...
        private int Send(String data)
        {
            ...
            try
            {
                ...
                //if (isConnected != true)
                //if (Volatile.Read(ref isConnected) == false)
                if (GetSendWorkSocket().Connected == false)
                {
                    //ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "SOCKET CLOSE", Thread.CurrentThread.ManagedThreadId);
                    //workSocket.Shutdown(SocketShutdown.Both);
                    //workSocket.Close();
                ...
                GetSendWorkSocket().Send(byteData, 0, byteData.Length, 0);
                ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "END", Thread.CurrentThread.ManagedThreadId);
                return 0;
        ...
        public int Send(byte[] data, int size)
        {
            ...
            try
            {
                ...
                GetSendWorkSocket().Send(data, 0, size, 0);
        ...
        private async void mesTimerHandler2(object sender, EventArgs e)
        {
            string className = "MESClient";
            ...
            {
                ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "START", Thread.CurrentThread.ManagedThreadId);
                mesRunningTimer.Interval = TimeSpan.FromSeconds(120);
                doingCmdFlag = true;
                mesRunningTimer.Stop();
                ...
                //1-1 Start MES Server
                Util.GetPrivateProfileValue("MES", "LISTENPORT", "64000", ref value, Constants.PARAMS_INI_FILE);
                int.TryParse(value, out serverPort);

                retval = await StartMESServer(IPAddress.Any, serverPort);
                if (retval != 0)
                {
                    ITNTMESLog.Instance.Trace(0, "{0}:{3:D4}:{1}()  {2}", className, funcName, "SERVER CONNECTION FAIL", Thread.CurrentThread.ManagedThreadId);
                    doingCmdFlag = false;
                    mesRunningTimer.Start();
                    //((DispatcherTimer)sender).Start();
                    ErrorCount++;

                    serverconnbefore = serverconnstatus;
                    serverconnstatus = csConnStatus.Closed;

                    if (serverconnstatus != serverconnbefore)
                    {
                        ServerStatusChangedEventArgs stsarg = new ServerStatusChangedEventArgs();
                        stsarg.oldstatus = serverconnbefore;
                        stsarg.newstatus = serverconnstatus;
                        ServerStatusChangedEvent?.Invoke(this, stsarg);
                    }
                    return;
                }
                ...
                if ((GetSendWorkSocket() == null) || (GetSendWorkSocket().Connected == false))
                {
                    ...
                    if (retval != 0)
                    {
                        ...
                        clientconnbefore = clientconnstatus;
                        clientconnstatus = csConnStatus.Closed;

                        if (clientconnstatus != clientconnbefore)
                        {
                            ServerStatusChangedEventArgs stsarg = new ServerStatusChangedEventArgs();
                            stsarg.oldstatus = clientconnbefore;
                            stsarg.newstatus = clientconnstatus;
                            ClientStatusChangedEvent?.Invoke(this, stsarg);
                        }
                        return;
                    }

                    clientconnbefore = clientconnstatus;
                    clientconnstatus = csConnStatus.Connected;
                ...
                {
                    ReceiveCommData((byte)MESRECVFLAG.RECVFLAG_INFO_START, MES_MSG_DATA);
                ...
                {
                    ErrorCount = 0;
                    mesDataInfo = await AnalyzeRequestDataMessage(recvFrame, MES_MSG_DATA);
                ...
                for (int i = 0; i < mesDataInfo.iblockCount; i++)
                {
                    ...
                    {
                        ReceiveCommData((byte)MESRECVFLAG.RECVFLAG_BLOCK_START, receiveSize);
                    ...
                    mesHeader = await SaveReceiveMessage(i, mesDataInfo.iblockCount, mesDataInfo.orderDate, recvFrame, iBlockSize + MES_MSG_DATA);
                ...
                ErrorCount = 0;
                value = mesDataInfo.orderDate.Trim();
                ...
                //접속 유지 현대 울산 51라인
                if ((GetSendWorkSocket() != null) && GetSendWorkSocket().Connected)
                {
        ...
        private void SetSendWorkSocket(Socket sc)
        {
            Volatile.Write(ref sendWorkSocket, sc);
        }

        private Socket GetSendWorkSocket()
        {
            return Volatile.Read(ref sendWorkSocket);
        }

        private void SetRecvWorkSocket(Socket sc)
        {
            Volatile.Write(ref recvWorkSocket, sc);
        }

        private Socket GetRecvWorkSocket()
        {
            return Volatile.Read(ref recvWorkSocket);
        }
